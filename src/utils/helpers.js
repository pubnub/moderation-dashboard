...
export const filterImageFunction = (data, key) => {
  if (data)
    return data.payload.filter(
      (item) => item.name === `KEY-${key.id}-IMAGE-MODERATION`
    );
};
...
export const filterImageEventHandler = (data, block) => {
  if (data)
    return data.filter(
      (item) => item.name === `BLOCK-${block[0].id}-IMAGE-MODERATION`
    );
};
...
export const combineLanguageWords = (wordList) => {
  const combinedWords = Object.values(wordList).filter(
    (item) => item.length !== 0
  );
...
export const formatBannedMessage = (messageObject) => {
  if (messageObject.file && messageObject.file.url)
    messageObject.image = messageObject.file.url;
