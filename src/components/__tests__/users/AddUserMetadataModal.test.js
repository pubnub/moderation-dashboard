...
describe('Add User meta data Modal', () => {
  ...
  it('should render pubnub and userinfo object correctly with given strings', () => {
    const component = shallow(
      <AddUserMetadataModal pubnubObject={pubnubObj} isAdded={userInfo} />
    );
    expect(component).toMatchSnapshot();
  });
  ...
  it('should check the Add User Form', () => {
    const component = shallow(
      <AddUserMetadataModal  isAdded={userInfo} />
    );
    component.find('AddUserForm').getElements()[0].props.formik.setFieldValue('name',userInfo[0].data.name);
    component.find('AddUserForm').getElements()[0].props.formik.setFieldValue('email',userInfo[0].data.email);
    component.find('AddUserForm').getElements()[0].props.formik.setFieldValue('user_id',userInfo[0].data.userId);
    component.find('AddUserForm').getElements()[0].props.formik.setFieldValue('profile_url',userInfo[0].data.profileUrl);
