...
describe('User List Test Cases', () => {
  ...
  test('delete user event', async () => {
    ...
    await act(async () => {
      // filter user
      expect(
        wrapper.find('FilterUsers').getElements()[0].props.isFiltered('all')
      ).toBeUndefined();
      expect(
        wrapper.find('FilterUsers').getElements()[0].props.isFiltered('one')
      ).toBeUndefined();
      // added user
      expect(
        wrapper.find('AddUserMetadataModal').getElements()[0].props.isAdded()
      ).toBeUndefined();
      // delete user 
