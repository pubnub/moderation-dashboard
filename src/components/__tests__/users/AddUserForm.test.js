...
describe('Add User Test Cases', () => {
  test('Snapshot', () => {
    const wrapper = shallow(
      <AddUserForm formik={props.formik} userAlert={props.userAlert} />
    );
  ...
  test('Labels and initial Values', () => {
    ...
    expect(wrapper.find("[id='user_id']").getElement().props.placeholder).toBe(
      'Enter user ID'
    );
    expect(wrapper.find("[id='name']").getElement().props.placeholder).toBe(
      'Enter user name'
    );
    expect(wrapper.find("[id='email']").getElement().props.placeholder).toBe(
      'Enter email address'
    );
    expect(
      wrapper.find("[id='profile_url']").getElement().props.placeholder
    ).toBe('Enter Profile URL');
  });
});
