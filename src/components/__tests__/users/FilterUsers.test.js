...
describe('Filter User meta data Modal', () => {
  ...
  test('Has Label', () => {
    ...
    expect(component.find('[testid="bannedUsers"]').text()).toBe(
      'Banned Users'
    );
    expect(component.find('[testid="flaggedUsers"]').text()).toBe(
      'Flagged Users'
    );
  });
  ...
  test('initial Values', () => {
    ...
    expect(
      component.find('input[type="checkbox"][checked="checked"]')
    ).toHaveLength(0);
  });
  ...
  test('check handleChange Function', () => {
    component
      .find('#banCheck')
      .simulate('change', { target: { checked: true, value: 'ban' } });
    ...
    component
      .find('#flagCheck')
      .simulate('change', { target: { checked: true, value: 'flag' } });
    ...
    component
      .find('#flagCheck')
      .simulate('change', { target: { checked: false, value: 'flag' } });
    expect(component.find('#flagCheck').getElement().props.checked).toBe(false);
    expect(component.find('#banCheck').getElement().props.checked).toBe(false);
  });
});
