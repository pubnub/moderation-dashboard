...
describe('This suit will test the rendering of overview page', () => {
  ...
  test('Overview Page Headings', () => {
    ...
    expect(wrapper.find("[testid='modified_date']").text()).toBe(
      'Modified Date'
    );
    ...
    expect(wrapper.find("[testid='subsciber_key']").text()).toBe(
      'Subscribe Key'
    );
  ...
  test('Overview Page Headings', () => {
    const wrapper = shallow(<OverviewGrid />);
    expect(
      wrapper.find('#maskableIcon').getElement().props.children.type.type.render
        .displayName
