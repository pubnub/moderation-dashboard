...
describe('Add User Test Cases', () => {
  ...
  beforeAll(() => {
    wrapper = shallow(
      <AddChannelForm formik={props.formik} channelAlert={props.channelAlert} />
    );
  });
  ...
  test('check placeholder', () => {
    expect(wrapper.find("[id='id']").getElement().props.placeholder).toBe(
      'Enter Channel ID'
    );
    ...
    expect(
      wrapper.find("[id='description']").getElement().props.placeholder
    ).toBe('Enter Channel Description');
  });
});
