...
describe('Test Cases for Channels Table', () => {
  ...
  test('check for editRow', () => {
    const mockData = [{}, {}];
    expect(
      wrapper.find('UpdateChannelMetadataModal').getElement().props.data
    ).toStrictEqual([]);
  ...
  test('check for handleRowClick', () => {
    ...
    wrapper
      .find('ListingTable')
      .getElement()
      .props.handleRowClick('', { id: 123 }, true);
    expect(mockPushFn).toHaveBeenCalledTimes(0);
    wrapper
      .find('ListingTable')
      .getElement()
      .props.handleRowClick('', { id: 123 }, false);
    expect(mockPushFn).toHaveBeenCalledTimes(1);
  });
