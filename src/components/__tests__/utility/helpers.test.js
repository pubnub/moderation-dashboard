...
describe('Test Cases For Helper', () => {
  ...
  test('check for pubnubChannelMembers, selectedChannel and uuid from local storage', () => {
    ...
    localStorage.setItem(
      'PubNubChannelMembers',
      JSON.stringify(mockPubnubChannelMembers)
    );
    ...
    localStorage.setItem(
      'PubNubSelectedChannel',
      JSON.stringify(mockPubNubSelectedChannel)
    );
  ...
  test('check for group Language Words, combineLanguageWords, truncateChannelDescription', () => {
    ...
    expect(defualturl).toBe('/images/default-user.svg');
  ...
  test('check for truncate String function', () => {
    ...
    expect(truncate50Str).toEqual(
      'Aenean commodo ligula eget dolor. Lorem ipsum do...'
    );
  });
  ...
  test('check for usersFromLS, channelsFromLS, applicationsWithKeyFromLS, selectedAppFromLS and selectedAccountsFromLS from local storage', () => {
    ...
    localStorage.setItem(
      'PubNubSelectedApp',
      JSON.stringify(mockPubNubApplications.result[0])
    );
    ...
    localStorage.setItem(
      'PubNubApplications',
      JSON.stringify(mockPubNubApplications)
    );
    ...
    localStorage.setItem(
      ...
      JSON.stringify(mockPubnubAccounts[1])
    );
  ...
  test('check for fetchWords function', async () => {
    ...
    expect(
      pnFunctionFilterStatus(`function runProfanity(request){return 'abc';}`)
    ).toBe('abc');
  });
  ...
  test('check for mute, block, message url', () => {
    ...
    expect(getMessageFileUrl(mockData, mockData.channelMessage)).toEqual(
      mockData
    );
