...
describe('Word List Text Moderation', () => {
  ...
  test('Contains Labels', () => {
    ...
    expect(wrapper.find('[testid="allChannelIdText"]').text()).toBe(
    ...
    expect(wrapper.find('[testid="defaultWordsLabel"]').text()).toBe(
      'Use Default words'
    );
    expect(wrapper.find('[testId="MaskOrBlock"]').text()).toBe(
      'When Profanity is detected'
    );
    expect(wrapper.find('[testId="maskLabel"]').text()).toBe(
      'Set a masking character'
    );
    expect(wrapper.find('[testid="ReRouteLabel"]').text()).toBe(
      'Route messages to'
    );
  });
  ...
  test('Check initial Values', () => {
    ...
    expect(wrapper.find('#wordListLanguage').getElement().props.value).toBe(
      'Hindi'
    );
    expect(wrapper.find('#defaultWordsBox').getElement().props.value).toBe(
      'Bakwaas, Chutiye'
    );
    expect(wrapper.find('#MaskOrBlockOption').getElement().props.value).toBe(
      'Mask-word'
    );
    ...
    expect(
      wrapper.find('#defaultWordsBox').getElement().props.placeholder
    ).toBe('Type @ to select parameters');

    expect(wrapper.find('#maskChar').getElement().props.placeholder).toBe(
      'Set'
    );
  });
  ...
  test('Check CheckBoxes', () => {
    expect(
      wrapper.find('#allChannelIdcheckBox').getElement().props.checked
    ).toBe(true);
    expect(wrapper.find('#reRouteCheckBox').getElement().props.checked).toBe(
      false
    );
  });
  ...
  test('Word Handler', () => {
    ...
    const WordListChange = wrapper
      .find('#MaskOrBlockOption')
      .simulate('change', {
    ...
    const AllChannelTrue = wrapper
      .find('#allChannelIdcheckBox')
      .simulate('change', {
        target: { checked: true },
      });
    expect(AllChannelTrue).toBeTruthy();

    const AllChannelFalse = wrapper
      .find('#allChannelIdcheckBox')
      .simulate('change', {
