...
describe('Test Case For Chat', () => {
  ...
  test('check functions', async () => {
    ...
    await act(async () => {
      expect(wrapper.find('MessageContent').getElements()[0].props.confirmDelete({preventDefault: jest.fn()},mockMessages)).toBeUndefined();
      expect(wrapper.find('MessageContent').getElements()[0].props.updateMessage({preventDefault: jest.fn()},mockMessages)).toBeUndefined();
      expect(wrapper.find('MessageContent').getElements()[0].props.confirmUndo({preventDefault: jest.fn()},mockMessages)).toBeUndefined();
