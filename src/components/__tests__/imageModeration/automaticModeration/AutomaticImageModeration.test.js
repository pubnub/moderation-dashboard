...
describe('Automatic Image Moderation Test Cases', () => {
  ...
  beforeAll(() => {
    wrapper = shallow(
      <AutomaticImageModeration state={mockState} setState={jest.fn()} />
    );
  });
  ...
  test('Contains Labels', () => {
    ...
    expectValue(
      wrapper.find('[testid="Apply_All"]').text(),
    ...
    expectValue(
      wrapper.find('[testId="ApiUser"]').text(),
      'Sightengine API User'
    );
    expectValue(
      wrapper.find('[testId="Api_Key"]').text(),
      'Sightengine API Key'
    );
    expectValue(
      wrapper.find('[testId="workFlowId"]').text(),
      'Sightengine Workflow ID'
    );
    expectValue(
      wrapper.find('[testId="riskFactor"]').text(),
      'Sightengine Risk Factor'
    );
    expectValue(
      wrapper.find('[testid="reRouteMsg"]').text(),
      'Route messages to'
    );
  });
  ...
  test('Check initial Values', () => {
    ...
    expect(wrapper.find('#ImageModerationTool').getElement().props.value).toBe(
      'sightengine'
    );
    ...
    expect(
      wrapper.find('#sightEngineWorkflowId').getElement().props.value
    ).toBe('');
    ...
    expect(
      wrapper.find('#sightEngineAPIUser').getElement().props.placeholder
    ).toBe('Sightengine API User');

    expect(wrapper.find('#apiKey').getElement().props.placeholder).toBe(
      'Sightengine API Key'
    );
    expect(
      wrapper.find('#sightEngineWorkflowId').getElement().props.placeholder
    ).toBe('Sightengine Workflow ID');
  });
  ...
  test('SightEngine Handler', () => {
    ...
    const ApiUserChange = wrapper
      .find('#sightEngineAPIUser')
      .simulate('change', {
    ...
    const WorkFlowIdChange = wrapper
      .find('#sightEngineWorkflowId')
      .simulate('change', {
